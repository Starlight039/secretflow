...
def prepare_data(args):
    ...
    for i in range(ld):
        traindataloader.append(
            torch.utils.data.DataLoader(
                traindata[i], batch_size=args.batch, shuffle=True
            )
        )
        testdataloader.append(
            torch.utils.data.DataLoader(
                testdata[i], batch_size=args.batch, shuffle=False
            )
        )
...
def pretrained(args, filename, device='cuda'):
    ...
    torch.save({'state': model.state_dict(), 'acc': acc}, filename)
    print('===done!===')
        ...
        with torch.no_grad():
            for data, _ in trainloadrs[i]:
                ...
                for item in fea:
                    if len(item.shape) == 4:
                        tm.append(
                            torch.mean(item, dim=[0, 2, 3]).detach().to('cpu').numpy()
                        )
                        tv.append(
        ...
        for client_idx in range(client_num):
            ...
            print(
                ' {:<11s}| Train Loss: {:.4f} | Train Acc: {:.4f}'.format(
                    datasetname[client_idx], train_loss, train_acc
                )
            )
        tmp = []
        for test_idx, test_loader in enumerate(test_loaders):
            ...
            print(
                ' {:<11s}| Test  Loss: {:.4f} | Test  Acc: {:.4f}'.format(
                    datasetname[test_idx], test_loss, test_acc
                )
            )
            tmp.append(test_acc)
...
def evaluate(args, datasetname, device, method):
    ...
    for i in range(client_num):
        ...
        print(
            ' {:<11s}| Test  Loss: {:.4f} | Test  Acc: {:.4f}'.format(
                datasetname[i], test_loss, test_acc
            )
        )
        accsum += test_acc
